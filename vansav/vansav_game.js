...
const CHARACTERS = [
    { id: 'roro', name: 'ロロ', desc: '主人公。平均的なステータス。', hp: 100, atk: 10, def: 5, spd: 100, luck: 1, sprite: 'player_down_1', unlocked: true },
    { id: 'lala', name: 'ララ', desc: '女の子。体力が少し低いが、素早く行動できる。', hp: 80, atk: 10, def: 6, spd: 130, luck: 2, sprite: 'player_left_1', unlocked: false }
];

const EQUIP_DATA = {
    // Attack
    magic_bullet: { type: 'atk', name: '魔法弾', icon: 'item_magic_bullet', maxLvl: 10 },
    sword: { type: 'atk', name: 'ソード', icon: 'item_sword', maxLvl: 10 },
    fireball: { type: 'atk', name: 'ファイヤーボール', icon: 'item_fireball', maxLvl: 10 },
    thunderbolt: { type: 'atk', name: 'サンダーボルト', icon: 'item_thunderbolt', maxLvl: 10 },
    poison_mist: { type: 'atk', name: 'ポイズンミスト', icon: 'item_poison_mist', maxLvl: 10 },
    // Buff
    scroll: { type: 'buf', name: '巻物', icon: 'item_scroll', maxLvl: 5 },
    magnet: { type: 'buf', name: '磁石', icon: 'item_magnet', maxLvl: 5 },
    magnifier: { type: 'buf', name: '拡大鏡', icon: 'item_magnifier', maxLvl: 5 }
};

const REQUIRED_SPRITES = [
    'item_magic_bullet', 'item_sword', 'item_fireball', 'item_thunderbolt', 'item_poison_mist',
    'item_scroll', 'item_magnet', 'item_magnifier', 'exp', 'item_boots', 'item_cross', 'item_coin_bag'
];
...
function loadSaveData() {
    ...
    CHARACTERS.find(c => c.id === 'lala').unlocked = SAVE_DATA.lalaUnlocked;
}
...
class Player {
    constructor(id, charData, x, y) {
        ...
        this.spriteNameBase = charData.sprite.split('_')[0];
        this.dir = 'down';
    ...
    get maxAtkSlots() { return GAME.is2P ? 2 : 3; }
...
class Enemy {
    constructor(x, y, type) {
        ...
        if (type === 'snakey') { this.hp = 20 * hpMultiplier; this.spd = 40; this.atk = 10; this.expDrop = 2; this.sprite = 'snakey_left'; }
        else if (type === 'medusa') { this.hp = 50 * hpMultiplier; this.spd = 30; this.atk = 15; this.expDrop = 5; this.sprite = 'medusa_awake'; this.ignoreWalls = true; }
        else if (type === 'gol') { this.hp = 100 * hpMultiplier; this.spd = 20; this.atk = 20; this.expDrop = 10; this.sprite = 'gol_down_awake'; }
        else if (type === 'don_medusa') { this.hp = 500 * hpMultiplier; this.spd = 25; this.atk = 30; this.expDrop = 50; this.sprite = 'don_medosa_1'; this.isBoss = true; }
        else if (type === 'bat') { this.hp = 10 * hpMultiplier; this.spd = 80; this.atk = 10; this.expDrop = 3; this.sprite = 'bat'; }
        else if (type === 'knight') { this.hp = 200 * hpMultiplier; this.spd = 15; this.atk = 30; this.expDrop = 15; this.sprite = 'knight'; }
        else { this.hp = 10; this.spd = 50; this.atk = 2; this.expDrop = 1; this.sprite = 'snakey_left'; }
        this.maxHp = this.hp;
        
        // Just generic sprite names mapping to assets.json logic if they don't exactly match
        // We will adapt the sprite drawing in render.
    }
}
...
function buildShopUI() {
    ...
    addItem('キャラ解放: ララ', 'player_left_1', 'unlock', 1, `キャラクターを追加`, SAVE_DATA.lalaUnlocked, () => { SAVE_DATA.lalaUnlocked = true; CHARACTERS.find(c => c.id === 'lala').unlocked = true; saveGameData(); });
}
...
function startGame() {
    ...
    for(let i=0; i<300; i++) {
        GAME.bgFeatures.push({
            type: Math.random()>0.5 ? 'tree':'wall_1',
    ...
    p1.equips.push({ id: 'magic_bullet', lvl: 1 }); // Starter weapon
    GAME.players.push(p1);
    
    if (GAME.is2P && GAME.p2SelectedChar) {
        let p2 = new Player(1, GAME.p2SelectedChar, 40, 0);
        p2.equips.push({ id: 'magic_bullet', lvl: 1 });
...
function checkWallCollision(x, y, radius) {
    for(let i=0; i<GAME.bgFeatures.length; i++) {
        ...
        if (f.x < x + radius && f.x + 32 > x - radius && f.y < y + radius && f.y + 32 > y - radius) {
            return true;
        }
    }
    return false;
    ...
    if (eq.id === 'magic_bullet') {
        ...
        for(let i=0; i<count; i++) {
            ...
            GAME.projectiles.push({
                ...
                life: 2, size: 10 * sizeMul, dmg: p.atk + eq.lvl*2, pierce: false, type: 'bullet'
            });
        }
        ...
        for(let i=0; i<count; i++) {
            ...
            GAME.projectiles.push({
                x: p.x, y: p.y, vx: Math.cos(ang)*100, vy: Math.sin(ang)*100,
                life: 5, size: 15 * sizeMul, dmg: p.atk + eq.lvl, pierce: true, type: 'fireball'
            });
        }
    }
...
function dropItem(e) {
    ...
    if (rand < 1 + luck) GAME.items.push(new Drop(e.x, e.y, 'heart', 1));
    else if (rand < 2 + luck) GAME.items.push(new Drop(e.x, e.y, 'cross', 1));
    else if (rand < 4 + luck) GAME.items.push(new Drop(e.x, e.y, 'coin_bag', Math.floor(GAME.time/60)+1));
...
function drawGame() {
    ...
    // Drops
    GAME.items.forEach(itm => {
        let spr = itm.type === 'exp' ? 'exp' : itm.type === 'heart' ? 'heart_normal' : itm.type === 'coin_bag' ? 'item_coin_bag' : itm.type === 'chest' ? 'chest_closed' : 'item_cross';
        if(PRE_RENDERED[spr]) ctx.drawImage(PRE_RENDERED[spr], itm.x - 16, itm.y - 16);
    });
    
    // Enemies
    GAME.enemies.forEach(e => {
        if(PRE_RENDERED[e.sprite]) ctx.drawImage(PRE_RENDERED[e.sprite], e.x - 16, e.y - 16);
        // HP Bar
        ctx.fillStyle = 'red'; ctx.fillRect(e.x - 16, e.y - 20, 32, 4);
        ctx.fillStyle = 'green'; ctx.fillRect(e.x - 16, e.y - 20, 32 * (e.hp/e.maxHp), 4);
    });
    ...
    GAME.projectiles.forEach(proj => {
        if (proj.type === 'fireball') ctx.fillStyle = 'orange';
...
function triggerLevelUp(p) {
    ...
    choices.forEach(cid => {
        ...
        div.innerHTML = `<div class="popup-icon"><canvas></canvas></div><div class="popup-desc"><b>${d.name}</b> (Lv.${nextLvl})<br>取得・強化します。</div>`;
        if (PRE_RENDERED[d.icon]) div.querySelector('canvas').getContext('2d').drawImage(PRE_RENDERED[d.icon],0,0);
        div.addEventListener('click', () => {
